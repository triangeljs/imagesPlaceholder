...
router.get('/', function(req, res, next) {
  ...
  var imgSrc = path.join(__dirname, '../', 'public/picCategory/', category, '/');
  ...
  if(getPicAttr['width']/getPicAttr['height'] <= setPicAttr['width']/setPicAttr['height']) {
    ...
    var buffer = images(getCurPic, 0, (setH - setPicAttr['height']) / 2, setPicAttr['width'], setPicAttr['height']).encode("jpg", {operation: 90});
  } else {
    var setW = Math.ceil(getPicAttr['width'] * setPicAttr['height'] / getPicAttr['height']);
    var getCurPic = curPic.size(setW);
    var buffer = images(getCurPic, (setW - setPicAttr['width']) / 2, 0, setPicAttr['width'], setPicAttr['height']).encode("jpg", {operation: 90});
  }
